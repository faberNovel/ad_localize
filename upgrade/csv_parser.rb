...
class CsvParser
  ...
  def parse_row(row)
    ...
    locales.each_with_object({ key_infos.dig(:key) => {} }) do |locale, output|
      ...
      if key_infos.dig(:numeral_key) == SINGULAR_KEY_SYMBOL
      ...
      elsif key_infos.dig(:numeral_key) == PLURAL_KEY_SYMBOL
  ...
  def parse_key(row)
    ...
    if plural_prefix.nil?
      numeral_key = SINGULAR_KEY_SYMBOL
    else
      numeral_key = PLURAL_KEY_SYMBOL
